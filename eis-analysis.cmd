...
if not exist "%PYTHON%" (
  ...
  if errorlevel 1 (
    echo [eis-analysis] Python 3.10 or newer is required. Please install Python 3.10+ and add it to PATH. 1>&2
    exit /b 1
  )
  echo [eis-analysis] First run: creating local Python environment... 1>&2
  !BASE_PY! -m venv "%~dp0.venv"
)

if not exist "%~dp0.venv\.eis_ecm_drt_installed" (
  echo [eis-analysis] First run: installing dependencies... 1>&2
  "%PYTHON%" -m pip install -r requirements.txt >NUL || goto install_failed
  echo installed>"%~dp0.venv\.eis_ecm_drt_installed"
)

"%PYTHON%" -m eis_ecm_drt.cli %*
exit /b %ERRORLEVEL%

:install_failed
echo [eis-analysis] Dependency installation failed. Check your internet connection or install requirements.txt manually. 1>&2
exit /b 1
